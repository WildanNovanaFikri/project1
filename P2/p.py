...
def tambahkan_pesanan_ke_keranjang():
    makanan_terpilih = makanan_var.get().split('. ')[1].split(' (')[0].strip()
    minuman_terpilih = minuman_var.get().split('. ')[1].split(' (')[0].strip()
    porsi_makanan = int(porsi_entry.get())
    jumlah_minuman = int(gelas_entry.get())

    pesanan = {
        'Makanan': makanan_terpilih,
        'Porsi': porsi_makanan,
        'Minuman': minuman_terpilih,
        'Jumlah Minuman': jumlah_minuman
    }
    
    keranjang.append(pesanan)
    # Update label untuk menampilkan ringkasan pesanan yang ada di keranjang
    update_label_keranjang()

def update_label_keranjang():
    keranjang_label.config(text="Pesanan dalam Keranjang:\n")
    for pesanan in keranjang:
        keranjang_label.config(text=keranjang_label.cget("text") +
                                      f"Makanan: {pesanan['Makanan']}, Porsi: {pesanan['Porsi']}, " +
                                      f"Minuman: {pesanan['Minuman']}, Jumlah Minuman: {pesanan['Jumlah Minuman']}\n")
        # Hitung total harga dari pesanan yang ada di keranjang
        hitung_total_pembelian_keranjang()

def hitung_total_pembelian_keranjang():
    total_pembelian_keranjang = 0
    for pesanan in keranjang:
        total_makanan = food_menu[pesanan['Makanan']] * pesanan['Porsi']
        total_minuman = drink_menu[pesanan['Minuman']] * pesanan['Jumlah Minuman']
        total_pembelian_keranjang += total_makanan + total_minuman
    
    total_pembelian_label_entry.delete(0, tk.END)
    total_pembelian_label_entry.insert(0, f"Rp {total_pembelian_keranjang}")

# Tambahkan tombol "Tambahkan Pesanan" ke antarmuka pengguna
tambah_pesanan_button = tk.Button(app, text="Tambahkan Pesanan", command=tambahkan_pesanan_ke_keranjang)
tambah_pesanan_button.pack()

# Tambahkan label untuk menampilkan pesanan dalam keranjang
keranjang_label = tk.Label(app, text="Pesanan dalam Keranjang:\n")
keranjang_label.pack()

# Update total pembelian setiap kali keranjang diperbarui
update_label_keranjang()
